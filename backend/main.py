...
origins = [
    get_env('FRONTEND_ORIGIN', 'http://localhost:3000'),  # React app address
]
...
def serialize_items(items):
    """Turn a list of IntegrationItem objects into JSON-friendly dicts."""
...
async def get_airtable_items(credentials: str = Form(...)):
    return serialize_items(await get_items_airtable(credentials))
...
async def get_notion_items(credentials: str = Form(...)):
    return serialize_items(await get_items_notion(credentials))
...
async def get_hubspot_items(credentials: str = Form(...)):
    return serialize_items(await get_items_hubspot(credentials))
