...
import base64
import requests
from config import get_env, redirect_uri
from integrations.integration_item import IntegrationItem

from redis_client import add_key_value_redis, get_value_redis, delete_key_redis

# Credentials are read from backend/.env - never hardcode secrets in the repo.
CLIENT_ID = get_env('NOTION_CLIENT_ID', 'XXX')
CLIENT_SECRET = get_env('NOTION_CLIENT_SECRET', 'XXX')
encoded_client_id_secret = base64.b64encode(f'{CLIENT_ID}:{CLIENT_SECRET}'.encode()).decode()

REDIRECT_URI = redirect_uri('notion')
